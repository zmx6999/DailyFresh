...
type User struct{ //用户表
	...
	PassWord 	string`orm:"size(64)"`		//登陆密码
...
func init()  {
	orm.RegisterDataBase("default","mysql","root:123456@tcp(localhost:3306)/db181112?charset=utf8")
	orm.RegisterModel(new(User),new(Address),new(Goods),new(GoodsSKU),new(GoodsType),new(GoodsImage),new(IndexGoodsBanner),new(IndexPromotionBanner),new(IndexTypeGoodsBanner),new(OrderInfo),new(OrderGoods))
